...
Multi-GPU (DataParallel):
  If more than one CUDA device is visible, the model is wrapped in
  nn.DataParallel. The batch is scattered across GPUs for the forward pass,
  and per-replica outputs (logits, f_proj, c_proj) are gathered back onto the
  primary device BEFORE the loss is computed. Because the InfoNCE loss runs on
  the gathered full batch, its in-batch negatives still span the entire batch
  (e.g. 32 or 64), so contrastive learning is fully preserved while letting you
  raise the batch size beyond what a single 2080 can hold.

  `batch_size` in config is the GLOBAL batch (split evenly across GPUs).
...
def unwrap(model: nn.Module) -> ImpactScoreModel:
    """Return the underlying ImpactScoreModel whether or not it is wrapped
    in nn.DataParallel. Use this whenever you need a custom method/attribute
    that DataParallel does not proxy (encode_*, unfreeze_top_layers,
    frozen_param_count, .encoder, .projection, .interaction, .attn_pool, ...).
    """
...
def build_optimizer(
    ...
    NOTE: always pass the UNWRAPPED ImpactScoreModel here (use unwrap(model)),
    never the DataParallel wrapper, so the .encoder/.projection/... attribute
    access below resolves correctly.
    """
...
def _maybe_mine_hard_negatives(
    ...
    NOTE: `model` here must be the UNWRAPPED ImpactScoreModel (the miner calls
    encode_* helpers that DataParallel does not proxy).
    """
    if miner is None:
...
def train(config: dict):
    ...
    use_dp = n_gpus > 1
    if use_dp:
        log.info("Detected %d CUDA devices — enabling nn.DataParallel.", n_gpus)
        log.info(
            "config batch_size=%d is the GLOBAL batch (≈%d per GPU). "
            "InfoNCE still runs on the full gathered batch.",
            tcfg["batch_size"], tcfg["batch_size"] // n_gpus,
        )
    else:
        log.info("Single-device training (%d CUDA device).", n_gpus)
    ...
    core    = build_model(config["model"]).to(device)
    loss_fn = build_loss(config["loss"])

    # Wrap for multi-GPU. `core` always points at the real ImpactScoreModel;
    # `model` is what we call in the forward path (DataParallel when n_gpus>1).
    model = nn.DataParallel(core) if use_dp else core

    frozen, total = core.frozen_param_count()
    ...
    for epoch in range(1, tcfg["total_epochs"] + 1):
        ...
        if miner is not None:
            ...
            train_loader, mined_this_epoch = _maybe_mine_hard_negatives(
                ...
                model              = core,   # miner needs the unwrapped model
                device             = device,
        ...
        # Save checkpoint (unwrap so state_dict keys are clean — no "module." prefix)
    ...
    if test_path and Path(test_path).exists():
        ...
        if not best_path.exists():
            ...
            # Load best weights into the underlying model (already on device)
